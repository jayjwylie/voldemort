 ...
 * 
 */
public class E2EClientRequestExecutorPoolTest {

    private static final String STORE_NAME = "test";
    private static final int NUM_CLIENTS = 2;
    private static final int NUM_PUTS = 20;
    private static final int NUM_EXEMPT_PUTS = 2;
    private static final long MAX_PUT_TIME_MS = 20;
    private static final long SLOW_PUT_MS = 100;
    /*
    ...
    private final SocketStoreFactory socketStoreFactory = new ClientRequestExecutorPool(1,
                                                                                        10 * 1000,
                                                                                        100 * 1000,
                                                                                        32 * 1024);
    ...
    public E2EClientRequestExecutorPoolTest() {
        this.useNio = true;
    }
    ...
    public void setUp() throws Exception {
        ...
        /*-
        Logger logger = Logger.getLogger("voldemort.utils.pool.KeyedResourcePool");
        logger.setLevel(Level.TRACE);
         */
        Logger logger = Logger.getLogger("voldemort.store.socket.SocketStore");
    ...
    public class Putter implements Runnable {
        ...
        // "Schedule" operations to happen with some periodicity...
        private void sleepForPeriod() {
        ...
        public void run() {
            for(int i = 0; i < puts; ++i) {
                ...
                System.out.println("OPERATION DONE  Time (ms): " + (endTime - startTime)
                                   + " (Thread: " + Thread.currentThread().getName() + ")");
                if(i >= NUM_EXEMPT_PUTS) {
                    // Exempt initial operations to account for lazily setting
                    // up the connection and warming it up.
                    assertFalse("Operation completes without blocking on slow server:"
                                        + (endTime - startTime),
                                (endTime - startTime) > this.putTimeLimitMs);
                }
            }
            sleepForPeriod();
    ...
    public void testPutToSameKey() {
        ...
        for(int i = 0; i < NUM_CLIENTS; i++) {
            ...
            // Key "3" uses node 2, a fast node as the master. This ensures that
            // the parallel put requests use node 1, the slow node. If checking
            // out a connection to a slow node is done asynchronously, then
            // operations should not block on the slow node before returning
            // (with secondary puts outstanding in the background).
            new Thread(new Putter(waitForPutters, NUM_PUTS, 3, false, MAX_PUT_TIME_MS)).start();
    ...
    public void testPutToDifferentKeys() {
        ...
        for(int i = 0; i < NUM_CLIENTS; i++) {
            System.out.println("THREAD CREATION");
            long timeout = MAX_PUT_TIME_MS + (NUM_CLIENTS * SLOW_PUT_MS);
            new Thread(new Putter(waitForPutters, NUM_PUTS, NUM_PUTS * i, true, timeout)).start();
        }
        System.out.println("POST THREAD CREATION");
        try {
            waitForPutters.await();
